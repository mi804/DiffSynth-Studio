...
import torch, os
from diffsynth.data.entity_controlled_text_image import ControlledTextImageDatasetMultiMask
...
class LightningModelForT2ILoRA(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        # Data
        text, image, masks, local_prompts = batch["text"], batch["image"], batch["masks"], batch["local_prompts"]
        ...
        local_prompts = [list(local_prompt) for local_prompt in local_prompts]
        local_prompts_transposed = [list(row) for row in zip(*local_prompts)]
        local_prompt_emb = [self.pipe.encode_prompt(local_prompt, positive=True) for local_prompt in local_prompts_transposed]
        local_prompt_emb = torch.stack([emb['prompt_emb'] for emb in local_prompt_emb])
        masks = F.interpolate(masks, size=(1, latents.shape[-2], latents.shape[-1]), mode='nearest')

        # Compute loss
        noise_pred = self.pipe.denoising_model()(
            noisy_latents, timestep=timestep, **prompt_emb, **extra_input, entity_prompt_emb=local_prompt_emb, entity_masks=masks,
