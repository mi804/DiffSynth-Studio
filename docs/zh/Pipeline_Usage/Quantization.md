...
| entropack | `pip install entropack>=0.3.0` | 尚未发布到 PyPI |
...
| `entropack_lattice_rans_bf16` | entropack | 有损 E8 格向量量化 BF16 / 不量化 | ✅ | ✅ |
...
- EntroPack 方法按需重构 BF16 权重。请用 `target_modules` / `exclude_modules` 显式选择层；有损方法按每行 8 维向量编码，要求权重为二维且列数可被 8 整除，不满足的层需显式排除。
