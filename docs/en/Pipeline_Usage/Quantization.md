...
| entropack | `pip install entropack>=0.3.0` | not published on PyPI yet |
...
- EntroPack methods reconstruct BF16 weights on demand. Select layers explicitly with `target_modules` / `exclude_modules`; the lossy method codes each row as 8-dim vectors, so weights must be 2D with a column count divisible by 8, and layers that do not qualify must be excluded.
